...
%if %{defined suse_version} && 0%{?sle_version} >= 150000
...
# workaround for sles. there seems to be no way to define this depencency without the exact version. This
# leads to a different package name for each sles release. The -devel pkg is not required, but depends on
# the lib pkg itself.
...
%if %{defined suse_version}
%service_add_pre gearmand.service
%endif
exit 0

%post
%if %{defined suse_version}
%service_add_post gearmand.service
...
%preun
%if %{defined suse_version}
%service_del_preun gearmand.service
%else
...
%postun
%if %{defined suse_version}
%service_del_postun gearmand.service
%else
%systemd_postun gearmand.service
%endif
